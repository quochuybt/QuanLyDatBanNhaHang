...
public class AssignShiftDialog extends JDialog {
    ...
    private JPanel createFormPanel() {
        ...
        cbCaLam.setRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
                ...
                if (value instanceof CaLam c) {
                    setText(String.format("%s (%s - %s)", c.getTenCa(), c.getGioBatDau().format(DateTimeFormatter.ofPattern("HH:mm")), c.getGioKetThuc().format(DateTimeFormatter.ofPattern("HH:mm"))));
                }
                return this;
            }
        });
        ...
        listNhanVien.setCellRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
                super.getListCellRendererComponent(list, value, index, isSelected, cellHasFocus);
                if (value instanceof NhanVien nv) setText(nv.getHoten() + " (" + nv.getManv() + ")");
                return this;
            }
        });
    ...
    private void loadData() {
        ...
        for (CaLam c : caLamService.findAll()) cbCaLam.addItem(c);

        DefaultListModel<NhanVien> model = (DefaultListModel<NhanVien>) listNhanVien.getModel();
        model.clear();
        for (NhanVien nv : nhanVienService.findAll()) model.addElement(nv);
    }
    ...
    private void assign() {
        ...
        if (ca == null || nvs.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn ca và ít nhất 1 nhân viên");
            return;
        }
        ...
        JOptionPane.showMessageDialog(this, "Thành công: " + ok + ", thất bại: " + fail);
        if (ok > 0) dispose();
    }
}
