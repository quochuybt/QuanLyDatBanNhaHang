...
public class NhanVienGUI extends BaseEventAwarePanel {
            ...
            switch (searchType) {
                case "Tên NV":
                    searchResults = allNV.stream()
                            .filter(nv -> nv.getHoten().toLowerCase().contains(keyword.toLowerCase()))
                            .toList();
                    break;
                case "SĐT":
                    searchResults = allNV.stream()
                            .filter(nv -> nv.getSdt() != null && nv.getSdt().contains(keyword))
                            .toList();
                    break;
            }
    ...
    private void updateTableWithResults(List<NhanVien> results) {
        ...
        // Tính tổng giờ làm từ PhanCong
        Map<String, Double> tongGioLamMap = tinhTongGioLam(currentMonth, currentYear);
