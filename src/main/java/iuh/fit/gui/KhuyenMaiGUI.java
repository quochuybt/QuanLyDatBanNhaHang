...
public class KhuyenMaiGUI extends JPanel {
    ...
    private void updateTable(List<KhuyenMai> ds) {
        ...
        for (KhuyenMai km : dsKhuyenMai) {
            String moTa = String.format("<html><b>%s</b><br>%s<br><i style='color:gray'>ĐK: >%.0f VNĐ</i></html>",
                    km.getTenChuongTrinh(), generateMoTaGiaTri(km), km.getDieuKienApDung());
            String ngayKT = (km.getNgayKetThuc() != null) ? km.getNgayKetThuc().format(dtf) : "Vô thời hạn";
            String soLuongHienThi = km.getSoLuotDaDung() + " / " + (km.getSoLuongGioiHan() > 0 ? km.getSoLuongGioiHan() : "∞");
    ...
    private String generateMoTaGiaTri(KhuyenMai km) {
        return switch (km.getLoaiKhuyenMai()) {
            case "Giảm theo phần trăm" -> String.format("Giảm %.0f%% hóa đơn", km.getGiaTri());
            case "Giảm giá số tiền" -> String.format("Giảm %.0f VNĐ", km.getGiaTri());
            default -> km.getMoTa();
        };
    }
    ...
    private JPanel createFooterPanel() { return new JPanel(); }
    ...
    private void thucHienTimKiemVaLoc() {
        ...
        if (!tuKhoa.isEmpty() && !"Tìm kiếm khuyến mãi".equals(tuKhoa)) {
            ketQua = ketQua.stream().filter(k ->
                    (k.getMaKM() != null && k.getMaKM().toLowerCase().contains(tuKhoa.toLowerCase())) ||
                    (k.getTenChuongTrinh() != null && k.getTenChuongTrinh().toLowerCase().contains(tuKhoa.toLowerCase()))
            ).collect(Collectors.toList());
        }
        if ("Đang áp dụng".equals(trangThai) || "Ngưng áp dụng".equals(trangThai)) {
            ketQua = ketQua.stream().filter(k -> trangThai.equals(k.getTrangThai())).collect(Collectors.toList());
        }
        updateTable(ketQua);
    }
    ...
    private void xoaKhuyenMai() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "Bạn có chắc chắn muốn ngưng áp dụng khuyến mãi này không?", "Xác nhận", JOptionPane.YES_NO_OPTION);
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                KhuyenMai km = dsKhuyenMai.get(tblKhuyenMai.convertRowIndexToModel(selectedRow));
                ...
                JOptionPane.showMessageDialog(this, "Cập nhật thất bại: " + ex.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
        ...
        tblKhuyenMai.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    ...
                    if (row != -1) showKhuyenMaiDialog(dsKhuyenMai.get(tblKhuyenMai.convertRowIndexToModel(row)));
                }
            }
        });
    }
    ...
    private void showKhuyenMaiDialog(KhuyenMai km) {
        JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), (km == null ? "Thêm Khuyến Mãi Mới" : "Cập Nhật Khuyến Mãi"), true);
        ...
        if (km != null && km.getNgayBatDau() != null) dcNgayBD.setDate(Date.valueOf(km.getNgayBatDau()));
        ...
        btnLuu.addActionListener(e -> {
            try {
                ...
                KhuyenMai kmMoi = new KhuyenMai(ma, ten, txtMoTa.getText(), (String) cbLoaiKM.getSelectedItem(), giaTri, dieuKien, ngayBD, ngayKT, (String) cbTrangThai.getSelectedItem());
                kmMoi.setSoLuongGioiHan(soLuongGioiHan);
                if (km != null) kmMoi.setSoLuotDaDung(km.getSoLuotDaDung());

                if (km == null) khuyenMaiService.add(kmMoi); else khuyenMaiService.update(kmMoi);
    ...
    private class TrangThaiRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
            ...
            label.setForeground("Đang áp dụng".equals(value) ? new Color(0, 150, 0) : COLOR_BUTTON_RED);
            return label;
        }
    }
}
