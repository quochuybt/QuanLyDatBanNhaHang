...
public class PhanCongAddHandler extends BaseCommandHandler implements CommandHandler {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(PhanCongAddHandler.class);

    private final PhanCongService phanCongService = new PhanCongService();
    private final SessionRegistry sessionRegistry;

    public PhanCongAddHandler(SessionRegistry sessionRegistry) {
    ...
    public MessageEnvelope handle(ClientSession session, MessageEnvelope request) {
        return execute(request, () -> {
            ...
            LOGGER.info("[SocketServer] PHANCONG_ADD thành công"
        ...
        }, "Lỗi server khi thêm phân công ca.");
    }
}
