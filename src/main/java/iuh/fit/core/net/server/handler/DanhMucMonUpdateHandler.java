...
public class DanhMucMonUpdateHandler extends BaseCommandHandler implements CommandHandler {
    ...
    public DanhMucMonUpdateHandler(SessionRegistry sessionRegistry) {
    ...
    public MessageEnvelope handle(ClientSession session, MessageEnvelope request) {
        return execute(request, () -> {
            ...
            boolean result = danhMucMonService.capNhatDanhMuc(payload);
            if (result) {
                sessionRegistry.broadcastBusinessEvent(
                        EventType.MENU_UPDATED, request.getName(),
                        "DANHMUCMON", payload.getMadm(), "UPDATED",
