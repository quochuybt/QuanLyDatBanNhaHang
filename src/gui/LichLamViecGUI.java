...
public class LichLamViecGUI extends JPanel {
    ...
    private void updateWeekRangeLabel() {
        ...
        lblWeekRange.setText(String.format(
                "Tuần: %s - %s",
    ...
    private JPanel createWeekPanel() {
        ...
        List<PhanCong> dsPhanCong = phanCongService.getPhanCongChiTiet(dauTuan, cuoiTuan);
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (phanCongCuaNgay != null) {
                caLamTheoNgay = phanCongCuaNgay.stream()
                        .filter(pc -> pc.getCaLam() != null && pc.getNhanVien() != null)
            ...
            String tenThu = ngayTrongTuan.getDayOfWeek()
                    .getDisplayName(TextStyle.FULL_STANDALONE, vn);
    ...
    private JPanel createDayColumn(
            String dayTitle,
            Map<CaLam, List<NhanVien>> caLamTheoNgay,
            LocalDate ngayLam
    ) {
        ...
        if (caLamTheoNgay != null && !caLamTheoNgay.isEmpty()) {
            List<CaLam> caLamSorted = caLamTheoNgay.keySet()
                    .stream()
            ...
            for (CaLam ca : caLamSorted) {
                ...
                String[] tenNhanViens = nhanViens.stream()
                        .map(nv -> nv.getHoten() != null ? nv.getHoten() : "Không rõ")
                        .toArray(String[]::new);
                ...
                pnlCardsContainer.add(createShiftCard(
                        thoiGian,
                        tenNhanViens,
                        ca,
                        nhanViens,
    ...
    private JPanel createShiftCard(
            String time,
            String[] employees,
            CaLam ca,
    ...
    private void showAssignShiftDialog() {
        AssignShiftDialog assignDialog = new AssignShiftDialog(
                (Frame) SwingUtilities.getWindowAncestor(this)
        );
